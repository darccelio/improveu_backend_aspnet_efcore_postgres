...
public class ExercicioService : IExercicioService
{
    ...
    {
        List<Exercicio> exercicios = await _context.Exercicios.Skip(skip).Take(take).ToListAsync();
    ...
    public async Task Atualizar(int id, Exercicio ex)
    {
        if (id <= 0)
        {
            throw new ArgumentException("Id do exercicio não pode ser menor ou igual a zero.");
        ...
        Exercicio? exercicio = _context.Exercicios.Find(id);

        if (exercicio == null)
        {
            throw new ArgumentException("Exercicio não encontrado.");
        }
